...
<html>
	...
	<body>
		...
		<form action="repaddreservation.jsp" method="POST">
			...
			ResultSet transitLine = st.executeQuery("SELECT DISTINCT transit_line FROM project.Train_new;");
			...
			<br><br><br>Customer Username: <input type="text" name="custuser"/><br/><br>
     	...
     	else 
     	{
			...
			if (disabled.length() != 0) 
			{
				...
				<a href = "repinsertreservation.jsp?trip=<%=trip%>&custuser=<%=custuser%>&transit_line=<%=transit_line%>&origin=<%=origin%>&destination=<%=destination%>
				&traveldate=<%=traveldate%>&traveltime=<%=traveltime%>&numadults=<%=numadults%>&numchildren=<%=numchildren%>
				&numseniors=<%=numseniors%>&numdisabled=<%=numdisabled%>&totalprice=<%=totalprice%>&classs=<%=classs%>"><button type="button">Confirm Reservation</button></a>	
